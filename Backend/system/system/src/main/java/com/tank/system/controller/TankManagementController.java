...
public class TankManagementController {
    ...
    public ResponseEntity<?> syncHardware(@RequestBody Map<String, Object> payload) {
        ...
        try {
            ...
            TankData tank = tankRepository.findByTankIdWithUser(id);
...
// C. AUTO FILL MODE
    ...
    public ResponseEntity<?> getUserTanks(@RequestParam String email) {
        return userRepository.findByEmail(email)
            .map(u -> {
                // Use findByUserEmail (derived query — no lazy user load needed)
                java.util.List<com.tank.system.model.TankData> tanks = tankRepository.findByUserEmail(email);
