...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function stopScanner() {
        // Hide overlay immediately so X button always works
        if (scannerOverlay) scannerOverlay.style.display = "none";
        if (scanStatus) { scanStatus.innerHTML = ""; }
        // Reset upload status too
        const uploadStatus = document.getElementById("upload-qr-status");
        if (uploadStatus) {
    ...
    async function onScanSuccess(decodedText) {
        ...
        try {
            ...
            if (checkData.registered) {
                ...
                if (scanStatus) scanStatus.innerHTML = "<span style='color:#dc2626;font-weight:600;'>ID Already Registered. Try a different QR.</span>";
            } else {
            ...
            // Server error — show message but keep X button fully working
            if (scanStatus) scanStatus.innerHTML = "<span style='color:#dc2626;font-weight:600;'>Server error. Close and try again.</span>";
    ...
    if (registerForm) {
        ...
        registerForm.addEventListener("submit", async (e) => {
            ...
            if (password.length < 6) {
                ...
                if (passField) {
                    passField.style.borderColor = "#dc2626";
                    passField.style.boxShadow = "0 0 0 2px rgba(220,38,38,0.2)";
                    let passErr = document.getElementById("pass-error-msg");
                    if (!passErr) { passErr = document.createElement("p"); passErr.id = "pass-error-msg"; passErr.style.cssText = "color:#dc2626;font-size:0.78rem;margin-top:4px;margin-bottom:0;"; passField.parentNode.insertBefore(passErr, passField.nextSibling); }
                    passErr.textContent = "Password must be at least 6 characters.";
                    passField.addEventListener("input", () => { passField.style.borderColor = ""; passField.style.boxShadow = ""; if (passErr) passErr.textContent = ""; }, { once: true });
                    passField.focus();
                }
                return;
            }
            if (password !== confirmPassword) {
                ...
                if (confirmField) {
                    ...
                    confirmField.style.boxShadow = "0 0 0 2px rgba(220,38,38,0.2)";
                    let confirmErr = document.getElementById("confirm-error-msg");
                    if (!confirmErr) { confirmErr = document.createElement("p"); confirmErr.id = "confirm-error-msg"; confirmErr.style.cssText = "color:#dc2626;font-size:0.78rem;margin-top:4px;margin-bottom:0;"; confirmField.parentNode.insertBefore(confirmErr, confirmField.nextSibling); }
                    confirmErr.textContent = "Passwords do not match. Please re-enter.";
                    confirmField.addEventListener("input", () => { confirmField.style.borderColor = ""; confirmField.style.boxShadow = ""; if (confirmErr) confirmErr.textContent = ""; }, { once: true });
                    confirmField.focus();
                }
                return;
            }
            ...
            try {
                ...
                if (regRes.ok) {
                    _notify("Account created! Redirecting to login...", "success");
                    setTimeout(() => {
                ...
                if (regRes.status === 409 || regRes.status === 400 || lower.includes("already") || lower.includes("email")) {
                    _notify("That email is already registered. Please log in instead.", "error");
                    const emailField = document.getElementById("regEmail");
                    if (emailField) {
                        emailField.style.borderColor = "#dc2626";
                        emailField.style.boxShadow = "0 0 0 2px rgba(220,38,38,0.2)";
                        let emailErr = document.getElementById("email-error-msg");
                        if (!emailErr) {
                            emailErr = document.createElement("p");
                            emailErr.id = "email-error-msg";
                            emailErr.style.cssText = "color:#dc2626;font-size:0.78rem;margin-top:4px;margin-bottom:0;";
                            emailField.parentNode.insertBefore(emailErr, emailField.nextSibling);
                        }
                        emailErr.textContent = "This email is already registered. Try logging in instead.";
        ...
        loginForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (response.status === 429) {
                    ...
                    _notify("No account found with this email. Please register first.", "error");
                    ...
                    // 401 — email exists but wrong password
                    _notify("Incorrect password. Please try again.", "error");
                }
            } catch (err) {
                console.error(err);
    ...
    // ── Upload QR Photo (Register Page) ─────────────────────────────────────
    const uploadQrBtn = document.getElementById("uploadQrBtn");
    const uploadQrInput = document.getElementById("uploadQrInput");
    const uploadQrStatus = document.getElementById("upload-qr-status");

    if (uploadQrBtn && uploadQrInput) {
        ...
        uploadQrInput.addEventListener("change", async () => {
            ...
            if (uploadQrStatus) {
                uploadQrStatus.style.display = "block";
                uploadQrStatus.style.color = "#888";
                uploadQrStatus.textContent = "Reading QR code from image...";
            }

            try {
                // Stop live camera first so it doesn't conflict
                if (html5QrCode) {
                    try {
                ...
                // Use a separate hidden div — never touches qr-reader (prevents face/photo showing in scanner)
                const TMP_ID = "qr-upload-tmp-div";
                ...
                // Canvas conversion degrades QR readability → causes "Invalid QR" on valid images.
                // This matches exactly how dashboard.js handles it.
                const decodedText = await fileScanner.scanFile(file, false);
                try { fileScanner.clear(); } catch (_) { }

                if (uploadQrStatus) {
                    uploadQrStatus.style.color = "#22c55e";
                    uploadQrStatus.textContent = "QR code detected! Verifying...";
                }

                // onScanSuccess fills tankInput and closes the modal — identical to direct camera scan
                await onScanSuccess(decodedText);

                // If still showing "Verifying..." after onScanSuccess (server error path), reset it
                if (uploadQrStatus && uploadQrStatus.textContent.includes("Verifying")) {
                    uploadQrStatus.style.color = "#dc2626";
                    uploadQrStatus.textContent = "Server error. Please try again or scan directly.";
                }
                uploadQrInput.value = "";
                ...
                if (uploadQrStatus) {
                    uploadQrStatus.style.color = "#dc2626";
                    uploadQrStatus.textContent = "Invalid QR code image. Please upload a clear QR photo.";
                }
                // Re-start the camera scanner so user can still scan directly after a failed upload
                try {
                    if (html5QrCode && scannerOverlay && scannerOverlay.style.display !== "none") {
                        html5QrCode = new Html5Qrcode("qr-reader");
                        html5QrCode.start(
                            { facingMode: "environment" },
                            { fps: 10, qrbox: { width: 250, height: 250 } },
    ...
    // ── End Upload QR Photo (Register Page) ─────────────────────────────────
});